...
class DQN(nn.Module):
    ...
    def __init__(self, input_dims=1, n_actions=4, learning_rate=1e-3, params_list=[32, 32], loss_fct='huber'):
        ...
        self.layers = []
        self.layers.append(nn.Linear(input_dims, params_list[0]))
        self.layers.append(nn.ReLU())
        self.layers.append(nn.Linear(params_list[0], params_list[1]))
        self.layers.append(nn.ReLU())
        self.layers.append(nn.Linear(params_list[1], n_actions))
        # for i in range(len(params_list)-1):
        #     if i == 0:
        #         self.layers.append(nn.Linear(input_dims, params_list[i+1]))
        #         self.layers.append(nn.ReLU())

        #     elif i == len(params_list)-2:
        #         # do not add ReLU for the last layer
        #         self.layers.append(nn.Linear(params_list[i], n_actions))
        #     else:
